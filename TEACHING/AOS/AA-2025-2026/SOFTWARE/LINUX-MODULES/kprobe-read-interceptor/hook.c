...
MODULE_DESCRIPTION("This module intercepts the return of the sys_read kernel function from standard-input\
...
//this function is executed at the startup of the kretprobed one - we record the address of the target user space buffer in a per thread location on the top of the stack area
static int the_pre_hook(struct kprobe *ri, struct pt_regs *the_regs) { 
	...
	addr = regs->si;
	printk("%s: pre-handler - target address is %px\n", MODNAME, (void*)addr);
...
static int the_hook(struct kretprobe_instance *ri, struct pt_regs *the_regs) { 
	...
	load_address(addr);
	if (addr == 0x0) goto end;//check if we saved the address of the user level buffer for the I/O operation
	...
	//now we restore the kprobe status to be managed
        preempt_disable();
#ifndef CONFIG_KRETPROBE_ON_RETHOOK
        __this_cpu_write(*temp, &ri->rp->kp);
#else
        __this_cpu_write(*temp, &the_retprobe->kp);
#endif
end:
	return 0;
