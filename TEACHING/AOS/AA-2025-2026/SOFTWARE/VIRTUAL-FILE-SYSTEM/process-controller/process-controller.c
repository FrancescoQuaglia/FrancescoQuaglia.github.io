...
static ssize_t hit_write(struct file *filp, const char *buff, size_t len, loff_t *off) {
  ...
  //do the below under RCU
  the_task = pid_task(find_vpid(pid),PIDTYPE_PID);
  if(!the_task){ 
  	rcu_read_unlock();
	return -ESRCH;//no such process
  }
  target_mm = get_task_mm(the_task);//this will also increment the reference counter
 				    //of the target mm structure
  ...
  current->mm = target_mm;//the_task->mm;//this is needed to correctly handle empty zero memory access or other faults
  _write_cr3(__pa(target_mm->pgd));
