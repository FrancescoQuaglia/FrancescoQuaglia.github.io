...
int main (int argc, char**argv){
	...
	AUDIT
	printf("secret area installed at address %lu\n",(unsigned long)secret_area);
	...
	while(1){
		...
		if(++j >= TRAINING_TRIALS){
			...
			printf("training over - average flush is %lu average no flush is %lu\n",average_flush, average_no_flush);
			threshold = average_no_flush + ((average_flush - average_no_flush)>>1) ; 

			printf("training over - threshold latency for cache hit/miss discrimination is %lu\n",threshold);
			break;
		}
	...
	AUDIT
	printf("probe buffer is at address %lu\n",(unsigned long)the_array);
	...
	for(j=0;j<BYTES_TO_LEAK;j++){
		...
		AUDIT
		printf("attacking at address %lu\n",(unsigned long)the_address);
