...
#define current_task (*((elem**)((void*)current->stack + sizeof(struct thread_info))))

elem head = {NULL,NO,-1,-1,-1,NULL,NULL};
elem tail = {NULL,NO,-1,-1,-1,NULL,NULL};
elem* last_running = NULL;
spinlock_t queue_lock; 

elem* NO_TASK = (elem*)"no stask";
...
asmlinkage long new_thread(unsigned int ip, unsigned int stack, int type){
...
void context_switch_handler(struct pt_regs *regs){
	...
	elem* next_task;
	long int ticks;
	int idle_sleep_disabled = 0;
	DECLARE_WAIT_QUEUE_HEAD(idle_sleep);

	if(regs->ax == 1) current_task = NO_TASK;
	...
	if(current_task != NO_TASK){
		memcpy((void*)((current_task)->context),(void*)regs,sizeof(struct pt_regs));
		(current_task)->running = NO;
	}

	if(regs->ax == 2){//del from runqueue
		(current_task)->next->prev = (current_task)->prev;
		(current_task)->prev->next = (current_task)->next;
		kfree((current_task)->context);
		kfree(current_task);
	...
	while(aux->next != &tail){
		...
		if((aux->next->ticks < ticks) && (aux->next->running == NO)){
			next_task = aux->next;
	...
	(current_task)->ticks++; 
	(current_task)->running = YES;

	AUDIT
        printk("%s: scheduling complete - restoring CPU context\n",MODNAME);
	memcpy((void*)regs,(void*)((current_task)->context),sizeof(struct pt_regs));

        spin_unlock(&queue_lock);
        preempt_enable();

	if((current_task)->idle == 1){
    	    wait_event_interruptible_timeout(idle_sleep,idle_sleep_disabled,SCALING);
	}
