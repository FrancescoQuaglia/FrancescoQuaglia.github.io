...
void *the_work(void * dummy){
	...
	printf("(%ld) variable x has value %d\n",me,x);
	printf("(%ld) TLS variable x has value %d\n",me,READ_THREAD_VARIABLE(x));

	WRITE_THREAD_VARIABLE(x,x*me);
	printf("(%ld) variable x has value %d\n",me,x);
	printf("(%ld) TLS variable x has value %d\n",me,READ_THREAD_VARIABLE(x));

	printf("(%ld) TLS variable v has value %ld\n",me,v);

	a = READ_THREAD_VARIABLE(k);
	printf("(%ld) TLS variable has k.x field set to %ld\n",me,(long int)a.x);
