...
void * worker(void* id){
	...
	if ((long)id == 0){
		for (j= 0; j<OPERATIONS ; j++)	{
			...
			AUDIT
			printf("thread %ld - inserting key %d - result %d\n",(long)id,j,ret);
		}	
		AUDIT
		printf("thread %ld - insertions done\n",(long)id);
		return NULL;
	}
	if ((long)id == 1){
		AUDIT
		...
		for (j= 0; j< 2*OPERATIONS ; j++)	{
			...
			AUDIT
			printf("thread %ld - removing key %d - result %d\n",(long)id,j,ret);
		}	
		AUDIT
		printf("thread %ld - removals done\n",(long)id);
		return NULL;
	}
	...
	for (j = 0; j < 2 * OPERATIONS ; j++)	{
		...
		AUDIT
		printf("thread %ld - scanning the list for key %d - result %d\n",(long)id,j,ret);
	}
