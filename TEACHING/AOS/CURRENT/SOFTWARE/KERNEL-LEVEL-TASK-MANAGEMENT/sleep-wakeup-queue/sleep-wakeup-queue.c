...
MODULE_DESCRIPTION("this module implements a sleep/wakeup FIFO queue - concurrent awakes can lead to awake the same thread");
...
asmlinkage long sys_goto_sleep(void){
	...
	preempt_disable();//actually redundant but stands here as an example of usage
...
sleep:
	...
	spin_unlock(&queue_lock);
	preempt_enable();//actually redundant but stands here as an example of usage
