...
int main(int argc, char** argv){
 ...
 printf("old program boundary is at address %lx (boundary page number is %ld)\n",(unsigned long)addr, (unsigned long)addr/(unsigned long)PAGE_SIZE);

 INCREMENT =  atoi(argv[1]);
 sbrk(INCREMENT);

 addr = (void*)sbrk(0);
 printf("new program boundary is at address %lx (boundary page number is %ld)\n",(unsigned long)addr, (unsigned long)addr/(unsigned long)PAGE_SIZE);
 ...
 for(i=0;;i++){ // eventually SEGFAULT
  printf("cycle %d accessing address %lx\n",i,(unsigned long)(oldboundary+i));
  fflush(stdout);
